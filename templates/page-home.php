...
<main id="main" class="site-main space-y-16 bg-surface py-12">
	<?php if ( have_posts() ) : ?>
		<?php while ( have_posts() ) : the_post(); ?>
			...
			<?php
			$tokens = function_exists( 'auriel_theme_get_design_tokens' ) ? auriel_theme_get_design_tokens() : array();
			if ( ! empty( $tokens ) ) :
				?>
				<section class="mx-auto max-w-5xl px-6">
					<h2 class="text-2xl font-semibold text-primary">
						<?php esc_html_e( 'Theme palette', 'auriel-theme' ); ?>
					</h2>
					<p class="mt-2 text-sm text-text/70">
						<?php esc_html_e( 'These values map to Tailwind utility classes via CSS variables.', 'auriel-theme' ); ?>
					</p>
					<div class="mt-6 grid gap-4 md:grid-cols-2">
						<?php foreach ( $tokens as $key => $value ) : ?>
							...
							<div class="flex items-center justify-between rounded-lg border border-text/10 bg-white/60 p-4 shadow-sm">
								...
								<span
									class="h-10 w-10 rounded-full border border-text/10 shadow-inner"
